...
class BoltzWriter(BasePredictionWriter):
    ...
    def write_on_batch_end(
        ...
        for record, z, pdistogram in zip(
            records,
            prediction["z"],
            prediction["pdistogram"],
        ):
...
class BoltzInterfaceWriter(BasePredictionWriter):
    """Writer for saving interface predictions."""
    ...
    def write_on_batch_end(
        ...
        """Write interface predictions on batch end.
        ...
        # Iterate over the records
        for record, interface_mask, interface_residues in zip(
            records,
            prediction["interface_mask"],
            prediction["interface_residues"],
        ):
            # Create output directory for this record
            record_dir = self.output_dir / record.id
            record_dir.mkdir(parents=True, exist_ok=True)

            # Save interface predictions as numpy arrays
            np.save(record_dir / "interface_mask.npy", interface_mask.cpu().numpy())

            # Save interface information as JSON
            interface_info = {
                "record_id": record.id,
                "num_chains": len(record.chains),
                "interface_mask_shape": list(interface_mask.shape),
                "total_interface_residues": int(interface_mask.sum().item()),
            }

            with open(record_dir / "interface_info.json", "w") as f:
                json.dump(interface_info, f, indent=2)

            print(f"Saved interface predictions for {record.id} to {record_dir}")

    def write_on_epoch_end(
        self,
        trainer: Trainer,  # noqa: ARG002
        pl_module: LightningModule,  # noqa: ARG002
        predictions: list[dict[str, torch.Tensor]],  # noqa: ARG002
        batch_indices: list[list[int]],  # noqa: ARG002
    ) -> None:
        """Write interface predictions on epoch end.
        ...
        """
        # Since we write on batch end, this method can be empty
        pass
        ...
        for record, coord, pad_mask in zip(records, coords, pad_masks):
            ...
            for model_idx in range(coord.shape[0]):
                ...
                if "plddt" in prediction:
                    ...
                    # Add pAEnergy if available
                    if "pae_energy" in prediction:
                        confidence_summary_dict["pae_energy"] = prediction["pae_energy"][model_idx].item()
                    
                    # Add pTMEnergy if available
                    if "ptm_energy" in prediction:
                        confidence_summary_dict["ptm_energy"] = prediction["ptm_energy"][model_idx].item()
                ...
                # Save pAEnergy
                if "pae_energy" in prediction:
                    pae_energy = prediction["pae_energy"][model_idx]
                    path = (
                        struct_dir
                        / f"pae_energy_{record.id}_model_{idx_to_rank[model_idx]}.npz"
                    )
                    np.savez_compressed(path, pae_energy=pae_energy.cpu().numpy())
                
                # Save pTMEnergy
                if "ptm_energy" in prediction:
                    ptm_energy = prediction["ptm_energy"][model_idx]
                    path = (
                        struct_dir
                        / f"ptm_energy_{record.id}_model_{idx_to_rank[model_idx]}.npz"
                    )
                    np.savez_compressed(path, ptm_energy=ptm_energy.cpu().numpy())
