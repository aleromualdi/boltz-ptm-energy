...
import boltz.model_impl.layers.initialize as init
from boltz.data import const
from boltz.data.mol import (
    minimum_lddt_symmetry_coords,
)
from boltz.model_impl.layers.pairformer import PairformerModule
from boltz.model_impl.loss.bfactor import bfactor_loss_fn
from boltz.model_impl.loss.confidencev2 import (
    confidence_loss,
)
from boltz.model_impl.loss.distogramv2 import distogram_loss
from boltz.model_impl.modules.affinity import AffinityModule
from boltz.model_impl.modules.confidencev2 import ConfidenceModule
from boltz.model_impl.modules.diffusion_conditioning import DiffusionConditioning
from boltz.model_impl.modules.diffusionv2 import AtomDiffusion
from boltz.model_impl.modules.encodersv2 import RelativePositionEncoder
from boltz.model_impl.modules.trunkv2 import (
    BFactorModule,
    ContactConditioning,
    DistogramModule,
    InputEmbedder,
...
from boltz.model_impl.optim.ema import EMA
from boltz.model_impl.optim.scheduler import AlphaFoldLRScheduler
...
class Boltz2(LightningModule):
    ...
    def forward(
        ...
        with torch.set_grad_enabled(
            ...
            # Check if we should return embeddings instead of continuing
            if hasattr(self, "predict_args") and self.predict_args.get(
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> dict:
        try:
            ...
            # Always include embeddings in the output
            ...
            # Check if we should also return structure predictions
            # If return_embeddings is True, only return embeddings
            # If return_embeddings is False, return both embeddings and structure predictions
            if not self.predict_args.get("return_embeddings", False):
                # Include structure predictions
                ...
                # Add interface predictions (always return them)
                pred_dict["interface_mask"] = out.get(
                    "token_interface_mask", out.get("atom_interface_mask", None)
                )
                pred_dict["interface_residues"] = batch.get(
                    "residue_pad_mask", batch.get("atom_pad_mask", None)
                )
                ...
                if self.confidence_prediction:
                    ...
                    if self.alpha_pae > 0:
                        ...
                        if "pae_logits" in out:
                            from boltz.energy import compute_pae_energy, compute_ptm_energy
                            pae_energy = compute_pae_energy(out["pae_logits"])
                            pred_dict["pae_energy"] = pae_energy
                            
                            # Compute pTMEnergy from PAE logits with chain information
                            if "asym_id" in batch:
                                chain_ids = batch["asym_id"]
                                ptm_energy = compute_ptm_energy(out["pae_logits"], chain_ids)
                                pred_dict["ptm_energy"] = ptm_energy
                if self.affinity_prediction:
